...
var attack_speed_multiplier = 0.3  # Замедление во время атаки (0.3 = 30% скорости)
...
func _ready():
	...
	print("Тяжёлый враг создан! Здоровье: ", health)
...
func _on_player_detected(body):
	if body.is_in_group("player"):
		...
		print("[Обнаружение] Тяжёлый враг: игрок ЗАМЕЧЕН!")

func _on_player_lost(body):
	if body.is_in_group("player"):
		player_detected = false
		print("[Обнаружение] Тяжёлый враг: игрок ПОТЕРЯН!")

func _on_player_entered_attack_range(body):
	if body.is_in_group("player"):
		player_in_range = true
		print("[Атака] Тяжёлый враг: игрок в зоне удара!")

func _on_player_exited_attack_range(body):
	if body.is_in_group("player"):
		player_in_range = false
		print("[Атака] Тяжёлый враг: игрок вышел из зоны удара")
...
func start_attack():
	print("[Атака] Тяжёлый враг: начинаю атаку!")
	is_attacking = true
	can_attack = false
	...
	if is_player_still_in_range():
		...
		print("[Атака] Тяжёлый враг: промах! Игрок вышел из зоны")
	...
	can_attack = true
	is_attacking = false
	print("[Атака] Тяжёлый враг: готов к новой атаке")
...
func deal_damage_to_player():
	print("[Атака] Тяжёлый враг: удар! Наношу урон ", attack_damage)
...
func take_damage(amount: int):
	health -= amount
	print("Тяжёлый враг: получил урон ", amount, ", осталось ", health)
